...
CONFIGS_DIR=$(readlink -f "${SCRIPT_DIR}/../src/s2ide_uprobe/configs")
QSERVER_DIR=$(readlink -f "${SCRIPT_DIR}/../src/s2ide_uprobe/qserver")
...
STARTUP_COMMAND="${PROCESS} --config=${QS_CONFIG_YML} --user-group-permissions=${QSERVER_DIR}/user_group_permissions.yaml"

# STARTUP_COMMAND="${PROCESS} --config=${QS_CONFIG_YML} \
#     --user-group-permissions-path=\"${CONFIGS_DIR}/user_group_permissions.yaml\" \
#     --existing-plans-and-devices-path=\"${CONFIGS_DIR}/existing_plans_and_devices.yaml\""
#--------------------
