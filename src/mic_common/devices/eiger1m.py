...
class Eiger1M(EigerDetectorCam):
    ...
    def scan_init(self, exposure_time, num_images, ptycho_exp_factor):
        ...
        if trigger_mode == "Internal Enable":
            yield from self.setup_internal_trigger(num_images)
            yield from self.set_trigger_exposure(exposure_time / ptycho_exp_factor)
        if trigger_mode == "Internal Series":
            yield from self.set_num_images(1)
            yield from self.set_num_triggers(1)
            yield from self.set_manual_trigger("Disable")
    ...
    def setup_internal_trigger(self, num_triggers):
        """
        Set up the internal enable trigger for the detector.

        This function will turn on manual trigger and set number of triggers
    ...
    @value_setter("trigger_exposure")
    def set_trigger_exposure(self, value):
        pass