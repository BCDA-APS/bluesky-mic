    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            if timing_data["total_calls"] % print_every == 0:
                ...
                print(
                    f"{description} {timing_data['total_calls']}: "
...
def loop_timer_context(description: str = "Loop", total_iterations: Optional[int] = None):
    ...
    class LoopTimer:
        ...
        def iteration(
            ...
            samy: Optional[float] = None,
            samx: Optional[float] = None,
            samz: Optional[float] = None,
            motorlist: Optional[list[Motor]] = None,
        ):
            ...
            if hasattr(self, "iteration_start"):
                ...
                if total_iterations:
                    ...
                    if self.motors is not None:
                        sam_str += " ".join(
                            [
                                f"{motor.name}: {motor.position:.4f} {motor.egu}, "
                    ...
                    print(
                        f"{description}: Line {self.current_iteration}/{total_iterations} "
                    ...
                    avg_time = sum(self.iteration_times) / len(self.iteration_times)
                    print(
                        f"{description}: Line {self.current_iteration} - "
