...
from s8bmb_uprobe.utils.nexus_bps_func import save_ophyd_value
...
def _common_flyscan_setup(
    ...
    xarr, x_start, x_end, num_pulses = (
        calculate_x_scan_parameters(width, x_center, stepsize_x, dwell_ms)
    )
    logger.info(
        f"x_start: {x_start}, x_end: {x_end}, num_pulses: {num_pulses}"
    )
...
def _fly2d_scanrecord(
    ...
    **kwargs,
    ...
    det_bools = [sis3820_on, xp3_on]
    det_names = ["sis3820", "xp3"]
