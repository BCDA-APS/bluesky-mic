...
from ..utils.fly import get_next_file_name
# from s2idd_uprobe.plans.toggle_usercalc import disable_usercalc, enable_usercalc
from ..utils.fly import validate_scan_parameters
from ..plans.stepscan_core import (
    _step1d_xrfnc,
...
def step2d(
    ...
    """Setup the sample x, and y position"""
    if x_center is None:
    ...
    """TODO: detector settings for step scans"""
    #TODO: if xpsress3: set num_images to 1, set trigger mode to 1 (internal trigger), set acquire time to dwell_ms
    #TODO: if mcs/scaler, MCS:scaler1.TP = dwell_ms, MCSscaler1.CONT = 0
    #TODO: if xmap, set spectra mode, set status passive, set read passive
    #TODO: if tetramm, set trigger mode, set acquire mode, set values per reading, set averaging time 
    ...
    devices_dict, fileplugins_dict = yield from _common_stepscan_setup(
        sis3820_on, xp3_on, tmm1_on, preamp1_on, preamp2_on, numpts_x, dwell_ms, filename
    )
