...
def _flysnake_scanrecord(
    ...
    **kwargs,
    ...
    """Execute before-outer sequence"""
    #NOTE: no need for before-scan busy record since this step ensures that the sample is at the start position
    #TODO: set busy records to DONE 
    if sample_z is not None:
        yield from bps.mv(sample.z, sample_z)
    if x_center is not None:
        yield from bps.mv(sample.x.direction, 0,
                          sample.x.offset, 0,
                          sample.x, x_center-width/2)
    if y_center is not None:
        yield from bps.mv(sample.y, y_center-height/2)

    """Set up inner / outer scan record based on the scan types and parameters"""
    det_bools = [sis3820_on, xp3_on, preamp1_on]
    det_names = ["sis3820", "xp3", "tmm1"]
    devices = validate_device_connections(det_bools, det_names, return_devices=True)
    yield from scanrecord.fly.stage2Dfly(sample, width, stepsize_x, height, stepsize_y)
    ...
    """Setup before-inner / after-inner busy sequences """
    #TODO: ready callback functions to perform following actions \
    #before inner busy:
    # if necessary, setup start/stop/send triggers  (2xfm hydra controller for example)
    # check stage in position / done moving 
    # for detector in detectors, make sure they are armed, but not acquiring
    # set before-inner busy to DONE with 0.15 seconds delay

    #after inner busy:
    # if necessary, setup start/stop/send triggers  (2xfm hydra controller for example)
    # check stage in position / done moving 
    # for detector in detectors, make sure they are done acquiring
    # if sample.x.DVAL == inner.end_position, set sample.x.direction to 1, else set to 0
    # if sample.x.DVAL == inner.end_position, set sample.x.offset to (inner.end_positon*2 - inner.width), else set to 0
    # set after-inner busy to DONE with 0.15 seconds delay

    """Start executing scan"""
    fname = savedata.next_file_name
    yield from scanrecord.fly.executeSnake2Dfly(scan_name=fname)

    """Execute after-scan sequence"""
    #NOTE: no need for after-scan busy record since this step ensures that settings are restored
    sample.x.set_speed(sample.x.get_max_velocity())
    sample.x.direction.put(0)
    sample.x.offset.put(0)
    yield from bps.checkpoint()
...
def _flysnake(
    ...
    **kwargs,
    ...
    """Main loop for the flysnake scan"""
