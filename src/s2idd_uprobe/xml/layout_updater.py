...
def update_meta_group(meta_group, attributes, json_lookup):
    ...
    print(
        f"Added {total_datasets} dataset elements organized into {len(grouped_attributes)} subgroups"
    )
    return True
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Update meta group in layout XML with attributes organized by groups"
    )
    parser.add_argument(
        "--attributes",
    ...
    parser.add_argument(
        "--json", default="savepv.json", help="Input savepv.json file (default: savepv.json)"
    )
    parser.add_argument(
        "--layout",
        default="2idd_tmm_layout.xml",
        help="Input layout XML file (default: 2idd_tmm_layout.xml)",
    )
    parser.add_argument(
        "--output",
        default="2idd_tmm_layout_updated.xml",
