...
def generate_random_points(
    scan_traj, x_center, y_center, width, height, stepsize_x, stepsize_y, dr, nth
):
    """
    ...
    if scan_traj == "spiral":
        scan_cyc = plan_patterns.spiral(
            "samx", "samy", x_center, y_center, width, height, dr, nth
        )
        samx_points, samy_points = process_scan_cyc(scan_cyc)

    elif scan_traj == "spiral_grid":
        scan_cyc = plan_patterns.spiral_square_pattern(
            "samx",
            "samy",
            x_center,
            y_center,
            width,
            height,
            int(width / stepsize_x),
            int(height / stepsize_y),
        )
        samx_points, samy_points = process_scan_cyc(scan_cyc)

    elif scan_traj == "grid":
        samx_points, samy_points = grid_points(
            x_center, y_center, width, height, stepsize_x, stepsize_y
        )
