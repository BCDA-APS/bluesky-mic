...
from isn.plans.utils.det_setup import ptycho_setup
from isn.plans.utils.det_setup import xrf_me7_setup
...
def step2d_random_pos(
    ...
    samx_points, samy_points = generate_random_points(
        scan_traj, 0, 0, width, height, stepsize_x, stepsize_y, dr, nth
    )
    ...
    if xrf_me7_on and xrf_me7.connected and xrf_me7_hdf.connected:
        ...
        yield from ptycho_setup(
            trigger_mode, num_capture, dwell, ptycho_exp_factor, filename
        )
    ...
    yield from bps.mv(
        scan1.positioners.p2.setpoint_pv, "", scan1.positioners.p2.readback_pv, ""
    )
