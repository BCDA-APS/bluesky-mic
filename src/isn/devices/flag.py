...
class Flag(DetectorBase):
    ...
    _default_read_attrs = ('stats1',)

    motor = FormattedComponent(EpicsMotor, "{motor_prefix}", labels=["baseline", "motor",])
    ...
    stats1 = FormattedComponent(MicStatsPlugin, "{flag_prefix}" + "Stats1:")
    stats2 = FormattedComponent(MicStatsPlugin, "{flag_prefix}" + "Stats2:")
    stats3 = FormattedComponent(MicStatsPlugin, "{flag_prefix}" + "Stats3:")
    stats4 = FormattedComponent(MicStatsPlugin, "{flag_prefix}" + "Stats4:")

    trans1 = FormattedComponent(TransformPlugin, "{flag_prefix}" + "Trans1:")
    ...
    def stage(self):
        # self.cam.acquire.put(0)     # stop any running acquisition before stage_sigs are applied
        return super().stage()      # applies stage_sigs, saves originals for restore on unstage

    def trigger(self):
        def detector_idle(value, old_value, **kwargs):
            # DetectorState_RBV: 0 = Idle — wait for a non-Idle → Idle transition
            return old_value != 0 and value == 0

        self.cam.acquire.put(1)
        return SubscriptionStatus(self.cam.detector_state, detector_idle, run=False)
