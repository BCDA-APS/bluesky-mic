"""
Miscelanous functions for users of instrument
"""
...
def mkdir(directory):
    """
    python function to create directory
    """
...
def mksubdirs(save_path, subdirs=[]):  # noqa: B006
    """
    python function to create sub-directory
    """
...
def run_subprocess(command_list):
    """
    python function to call custom terminal command
    """
    try:
...
def resume_scan():
    """
    python function to resume scan
    """
    from ..devices.scan_record import scan2
...
def scan_number_in_list(lst, partial_str):
    """
    python function scan number in list
    """
    ...
    if len(matches) > 1:
        ...
        print(
            "warning, no matches found, check that files are being saved and closing correctly"  # noqa: E501
        )
    elif len(matches) == 1:
        pass
    else:
...
def create_master_file(
    ...
    sample_name,
    scan_number,
    groups=["flyXRF", "eiger", "mda", "tetramm", "positions"],  # noqa: B006
):
    """
    python function create master file
    """
    # TODO: samples with the same name get saved to the same folder even if separate
    # scan, need to link files based on scan number instead of all files in folder.
    with h5py.File(f"{basedir}/{sample_name}_{scan_number}_master.h5", "w") as f:
        for group in groups:
            ...
            if group == "tetramm":
                ...
                file = scan_number_in_list(files, str(scan_number))
                string_data = [file.encode("utf-8")]
                dset = f[group].create_dataset("fnames", data=string_data)  # noqa: F841
            elif group == "flyXRF":
