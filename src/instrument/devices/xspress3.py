...
class Xspress3(Device):
    ...
    def initialize(self, file_path=None, file_prefix=None, file_number=None,
                      file_format="%s%s_%05d.h5", trigger_mode=3):
    ...
    def setup_xspress3(self, npts, dwell_time):
        ...
        if self.status == "Idle":
            ...
            if not self.Capture_RBV.get():
                ...
                logger.info(f"Xspress3 is ready to start")
            else:
                logger.error(f"Not able to arm the detector properly")
        else:
            logger.warning(f"Xspress3 not able to change state")
