...
def write_master_h5(
    ...
    with h5py.File(master_path, "w") as f:
        for det_name, file_ext, det_k in zip(det_names, det_file_ext, det_key, strict=False):
            ...
            det_dir = os.path.join(*[basedir, det_name, sample_name])
            files = [fn for fn in os.listdir(det_dir) if file_ext in fn]
            if file_ext != ".nc":
                for i, fn in enumerate(files):
                    group[f"{fn}"] = h5py.ExternalLink(os.path.join(*[basedir, det_name, sample_name, fn]), det_k)
            else:
                group[f"{fn}"] = os.path.join(*[basedir, det_name, sample_name, fn])
