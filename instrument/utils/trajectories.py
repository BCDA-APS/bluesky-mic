...
def raster_sr(dwell, step_size, x_center, y_center, x_width, y_width):
    npts_y = int(np.ceil(y_width/step_size))
    ypts = np.linspace(y_center-y_width/2,y_center+y_width/2, npts_y)
    npts_line = int(np.ceil(x_width/step_size))
    xpts = [x_center - x_width / 2, x_center + x_width / 2]
    x = np.asarray(xpts)
    y = np.asarray(ypts)
    npts_tot = npts_y*npts_line
    return x, y, npts_line, npts_tot
