...
def setup_scan1(scan1, scanrecord_name, ioc, m1_name, m2_name,
                m1_start, m1_finish, m2_start, 
                m2_finish, npts, dwell_time):
    ...
    yield from bps.mv(
        ...
        scan1.positioners.p1.readback_pv, f"{ioc}{m1_name}.RBV",
        scan1.positioners.p1.setpoint_pv, f"{ioc}{m1_name}",
        scan1.positioners.p2.mode, 1,
        scan1.positioners.p2.readback_pv, f"{ioc}{m2_name}.RBV",
        scan1.positioners.p2.setpoint_pv, f"{ioc}{m2_name}",
        scan1.positioner_delay, dwell_time,
        scan1.number_points, npts_tot
    )
    print("exit in setup_scan function")

def scan_record_isn(scanrecord_name = 'scan1', ioc = "2idsft:", m1_name = 'm1',
                   m1_start = -0.5, m1_finish = 0.5,
                   m2_name = 'm3', m2_start = -0.2 ,m2_finish = 0.2, 
                   npts = 50, dwell_time = 0.1):
    
    """Simple bluesky plan for demonstrating 2D raster scan."""
    param_labels = ['scanrecord_name', 'ioc', 'm1_name', 'm1_start', 'm1_finish',
                    'm2_name', 'm2_start', 'm2_finish', 'npts', 'dwell_time']
    ...
    yield from setup_scan1(scan1, scanrecord_name, ioc, m1_name, m2_name,
                           m1_start, m1_finish,
                           m2_start, m2_finish, 
                           npts, dwell_time)
